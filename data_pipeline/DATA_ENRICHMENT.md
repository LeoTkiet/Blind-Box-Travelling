...
1. Lấy API Key miễn phí từ [Groq Console](https://console.groq.com) (cho Groq Analyst). (30 req/phút).
2. Lấy API Token miễn phí từ [Pollinations](https://auth.pollinations.ai) (cho Pollinations Supervisor). (1 req/5s).
3. (Tùy chọn) Lấy User ID làm API Key từ [G4F](https://g4f.dev/api_key.html) để làm lớp dự phòng phòng khi Groq/Pollinations sập hàng loạt.
4. Copy định dạng JSON sau dán vào file (có thể nhét nhiều key tùy ý để script tự luân phiên):
...
3. **Smart Search Tags xịn xò:** Tags được uốn nắn giữ nguyên dấu tiếng Việt chuẩn chỉnh, diệu kỳ qua bộ lọc Synonym (Đồng nghĩa) và Stopwords để vắt bớt mỡ thừa token cực ngắn nhưng thâu tóm "Giá trị check-in" tuyệt hảo.
4. **Enrich Metadata Gọn Nhẹ:** Dữ liệu hoàn chỉnh được ghi cuộn và đẩy trực tiếp xuống thẳng `data_enriched.json` mỗi vòng mà không lo tràn RAM bùng nổ, ghi nhận minh bạch `aiSource` là nguồn nào đã thực thi.
